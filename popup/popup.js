...
function initHeader() {
  ...
  $('settingsBtn').addEventListener('click', () => {
    selectTab('colour', true);
    requestAnimationFrame(() => {
      $('websiteSettingsCard')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });
}
...
async function savePendingCapture(captureRecord) {
  try {
    ...
    try {
      ...
      console.warn('[LCBC popup] pending capture save failed', inlineError);
...
function buildThemeCard(theme) {
  ...
  preview.style.background = `linear-gradient(135deg, ${theme.background} 0%, ${theme.accent} 52%, ${theme.secondaryAccent || theme.accent} 100%)`;
