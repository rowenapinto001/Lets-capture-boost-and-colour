...
function getThemeSecondaryColor(theme, fallback) {
  return Utilities.isValidHex(theme.secondaryAccent) ? theme.secondaryAccent : fallback;
}
...
function buildLightGradients(theme) {
  ...
  const secondaryColor = getThemeSecondaryColor(theme, color);
  const hsl = hexToHsl(color);
  const secondaryHsl = hexToHsl(secondaryColor);
  const accentSat = Utilities.clamp(Math.max(hsl.s, 68), 58, 96);
  const secondaryAccentSat = Utilities.clamp(Math.max(secondaryHsl.s, 68), 58, 96);
  ...
  const secondaryStrongGlow = hslFromBase(secondaryHsl, secondaryAccentSat, 58);
  const secondarySoftGlow = hslFromBase(secondaryHsl, Utilities.clamp(secondaryAccentSat - 6, 58, 92), 68);
  const secondaryPaleGlow = hslFromBase(secondaryHsl, Utilities.clamp(secondaryAccentSat - 22, 42, 76), 82);
  ...
  const gradientMid = Utilities.mixColors(Utilities.mixColors(pageBackground, color, 0.1), secondaryColor, 0.1);
  const gradientEnd = Utilities.mixColors(secondaryBackground, secondaryColor, 0.34);
  ...
  const secondarySelectedEnd = hslFromBase(
    secondaryHsl,
    Utilities.clamp(secondaryAccentSat - 18, 44, 82),
    88
  );
  const secondaryButtonEnd = hslFromBase(secondaryHsl, secondaryAccentSat, 42);

  return {
    screenGradient: [
      `linear-gradient(90deg, ${rgbaFromHex(strongGlow, 0.5)} 0%, ${rgbaFromHex(softGlow, 0.34)} 20%, transparent 46%)`,
      `linear-gradient(270deg, ${rgbaFromHex(secondaryStrongGlow, 0.45)} 0%, ${rgbaFromHex(secondarySoftGlow, 0.3)} 20%, transparent 48%)`,
      ...
      `radial-gradient(circle at 68% 6%, ${rgbaFromHex(secondaryStrongGlow, 0.52)} 0, transparent 30rem)`,
      `radial-gradient(circle at 86% 12%, ${rgbaFromHex(secondarySoftGlow, 0.64)} 0, transparent 32rem)`,
      `radial-gradient(circle at 50% 56%, ${rgbaFromHex(Utilities.mixColors(paleGlow, secondaryPaleGlow, 0.5), 0.54)} 0, transparent 44rem)`,
      `linear-gradient(135deg, ${gradientStart} 0%, ${gradientMid} 46%, ${gradientEnd} 100%)`
    ].join(', '),
    headerGradient: `linear-gradient(135deg, ${Utilities.mixColors(surface, color, 0.08)} 0%, ${elevatedSurface} 48%, ${Utilities.mixColors(hoverBackground, secondaryColor, 0.18)} 100%)`,
    sidebarGradient: `linear-gradient(180deg, ${Utilities.mixColors(sidebarBackground, color, 0.16)} 0%, ${Utilities.mixColors(pageBackground, secondaryColor, 0.18)} 100%)`,
    surfaceGradient: `linear-gradient(145deg, ${Utilities.mixColors(surface, color, 0.06)} 0%, ${elevatedSurface} 58%, ${Utilities.mixColors(subtleAccentSurface, secondaryColor, 0.14)} 100%)`,
    cardGradient: `linear-gradient(145deg, ${cardBackground} 0%, ${Utilities.mixColors(surfaceTint, secondaryColor, 0.1)} 62%, ${surface} 100%)`,
    menuGradient: `linear-gradient(145deg, ${Utilities.mixColors(menuBackground, color, 0.06)} 0%, ${Utilities.mixColors(surfaceTint, secondaryColor, 0.12)} 58%, ${surface} 100%)`,
    inputGradient: `linear-gradient(135deg, ${inputBackground} 0%, ${Utilities.mixColors(elevatedSurface, secondaryColor, 0.1)} 100%)`,
    selectedGradient: `linear-gradient(135deg, ${selectedBackground} 0%, ${secondarySelectedEnd} 100%)`,
    buttonGradient: `linear-gradient(135deg, ${theme.strongAccent || color} 0%, ${secondaryButtonEnd} 100%)`
  };
}
...
function buildDarkGradients(theme, options = {}) {
  ...
  const headerEnd = Utilities.mixColors(theme.moderatelyDark || theme.headerBackground || surface, secondaryAccent, 0.34);
  const sidebarEnd = Utilities.mixColors(theme.dark || pageBackground, secondaryAccent, 0.18);
  const surfaceEnd = Utilities.mixColors(theme.moderatelyDark || surface, secondaryAccent, 0.2);
  const cardEnd = Utilities.mixColors(theme.subtleAccentSurface || cardBackground, secondaryAccent, 0.18);
  const menuEnd = Utilities.mixColors(theme.elevatedSurface || menuBackground, secondaryAccent, 0.22);
  const inputEnd = Utilities.mixColors(inputBackground, secondaryAccent, isAmoled ? 0.14 : 0.18);
  const selectedEnd = Utilities.mixColors(selectedBackground, secondaryAccent, 0.48);
  const buttonEnd = Utilities.mixColors(buttonBackground, secondaryAccent, 0.7);
  ...
  return {
    screenGradient: [
      ...
      `radial-gradient(circle at 82% 10%, ${rgbaFromHex(secondaryAccent, isAmoled ? 0.36 : 0.3)} 0, transparent 34rem)`,
      `radial-gradient(circle at 50% 46%, ${rgbaFromHex(strongAccent, isAmoled ? 0.16 : 0.14)} 0, transparent 42rem)`,
      `linear-gradient(135deg, ${deepestBackground} 0%, ${pageBackground} 48%, ${Utilities.mixColors(theme.dark || pageBackground, secondaryAccent, 0.16)} 100%)`
    ].join(', '),
    ...
    buttonGradient: `linear-gradient(135deg, ${buttonBackground} 0%, ${buttonEnd} 100%)`
  };
}
...
function generateDarkTheme(baseColor, options = {}) {
  ...
  const secondaryColor = getThemeSecondaryColor(base, color);
  const mode = options.mode === 'amoled' || options.amoled ? 'amoled' : 'dark';
  const hsl = hexToHsl(color);
  const secondaryHsl = hexToHsl(secondaryColor);
  const accentSat = Utilities.clamp(Math.max(hsl.s, 78), 70, 96);
  const secondaryAccentSat = Utilities.clamp(Math.max(secondaryHsl.s, 78), 70, 96);
  const bgSat = Utilities.clamp(Math.max(hsl.s * 0.56, 42), 34, 64);
  const secondaryBgSat = Utilities.clamp(Math.max(secondaryHsl.s * 0.56, 42), 34, 64);
  ...
  const secondaryAccent = hslFromBase(secondaryHsl, secondaryAccentSat, Utilities.clamp(Math.max(secondaryHsl.l, 56), 52, 65));
  ...
  const secondaryBrightAccent = hslFromBase(secondaryHsl, Utilities.clamp(secondaryAccentSat + 2, 78, 100), 68);
  const selectedBackground = hslFromBase(hsl, Utilities.clamp(accentSat, 72, 100), isAmoled ? 35 : 34);
  const secondarySelectedBackground = hslFromBase(secondaryHsl, Utilities.clamp(secondaryAccentSat, 72, 100), isAmoled ? 35 : 34);
  const buttonBackground = hslFromBase(hsl, Utilities.clamp(accentSat, 74, 100), isAmoled ? 48 : 46);
  const secondaryButtonBackground = hslFromBase(secondaryHsl, Utilities.clamp(secondaryAccentSat, 74, 100), isAmoled ? 48 : 46);
  const gradientLow = hslFromBase(hsl, bgSat, isAmoled ? 4 : 7);
  const secondaryGradientLow = hslFromBase(secondaryHsl, secondaryBgSat, isAmoled ? 4 : 7);
  ...
  const screenGradient = [
    `linear-gradient(90deg, ${rgbaFromHex(accent, isAmoled ? 0.16 : 0.13)} 0%, ${rgbaFromHex(strongAccent, isAmoled ? 0.1 : 0.085)} 18%, transparent 44%)`,
    `radial-gradient(circle at 20% 0%, ${rgbaFromHex(brightAccent, isAmoled ? 0.34 : 0.3)} 0, transparent 38rem)`,
    `radial-gradient(circle at 82% 10%, ${rgbaFromHex(secondaryBrightAccent, isAmoled ? 0.36 : 0.31)} 0, transparent 34rem)`,
    `radial-gradient(circle at 50% 46%, ${rgbaFromHex(strongAccent, isAmoled ? 0.16 : 0.14)} 0, transparent 42rem)`,
    `linear-gradient(135deg, ${deepestBackground} 0%, ${Utilities.mixColors(pageBackground, gradientLow, 0.5)} 48%, ${secondaryGradientLow} 100%)`
  ].join(', ');
  const headerGradient = `linear-gradient(135deg, ${hslFromBase(hsl, surfaceSat, isAmoled ? 8 : 13)} 0%, ${Utilities.mixColors(gradientHigh, secondaryAccent, 0.38)} 100%)`;
  const sidebarGradient = `linear-gradient(180deg, ${hslFromBase(hsl, surfaceSat, isAmoled ? 7 : 12)} 0%, ${hslFromBase(secondaryHsl, secondaryBgSat, isAmoled ? 4.8 : 9)} 100%)`;
  const surfaceGradient = `linear-gradient(145deg, ${hslFromBase(hsl, surfaceSat, isAmoled ? 5 : 12)} 0%, ${hslFromBase(secondaryHsl, secondaryBgSat, isAmoled ? 8.5 : 17)} 100%)`;
  const cardGradient = `linear-gradient(145deg, ${hslFromBase(hsl, surfaceSat, isAmoled ? 4.5 : 10.5)} 0%, ${Utilities.mixColors(gradientMid, secondaryAccent, 0.24)} 100%)`;
  const menuGradient = `linear-gradient(145deg, ${hslFromBase(hsl, surfaceSat, isAmoled ? 7 : 13)} 0%, ${hslFromBase(secondaryHsl, secondaryBgSat, isAmoled ? 10.5 : 19)} 100%)`;
  const inputGradient = `linear-gradient(135deg, ${hslFromBase(hsl, bgSat, isAmoled ? 3.8 : 8)} 0%, ${hslFromBase(secondaryHsl, secondaryBgSat, isAmoled ? 6.5 : 12)} 100%)`;
  const selectedGradient = `linear-gradient(135deg, ${selectedBackground} 0%, ${secondarySelectedBackground} 100%)`;
  const buttonGradient = `linear-gradient(135deg, ${buttonBackground} 0%, ${secondaryButtonBackground} 100%)`;
